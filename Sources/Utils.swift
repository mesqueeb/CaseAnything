...
func magicSplit() -> NSRegularExpression {
  return try! NSRegularExpression(
    pattern:
      "^[a-zà-öø-ÿа-я]+|[A-ZÀ-ÖØ-ßА-Я][a-zà-öø-ÿа-я]+|[a-zà-öø-ÿа-я]+|[0-9]+|[A-ZÀ-ÖØ-ßА-Я]+(?![a-zà-öø-ÿа-я])"
  )
}
...
func splitAndPrefix(
  ...
  let normalString = string.trimmingCharacters(in: .whitespacesAndNewlines)
  ...
  return zip(partsAndIndexes.parts, partsAndIndexes.prefixes).enumerated()
    .compactMap { index, element in
      ...
      if !keepSpecialCharacters || !keep.isEmpty {
        ...
        if let regex = try? NSRegularExpression(pattern: regexPattern) {
          ...
          part = regex.stringByReplacingMatches(
            in: partDecomposed,
            range: NSRange(0 ..< partDecomposed.utf16.count),
            withTemplate: ""
          )
      ...
      if !keep.isEmpty, !foundPrefix.isEmpty {
        ...
        if let regex = try? NSRegularExpression(pattern: regexPattern) {
          foundPrefix = regex.stringByReplacingMatches(
            in: foundPrefix,
            range: NSRange(0 ..< foundPrefix.utf16.count),
            withTemplate: ""
          )
        }
      }
      ...
      if index == 0 { return foundPrefix + part }

      if foundPrefix.isEmpty, part.isEmpty { return "" }

      if !hasSpaces { return (foundPrefix.isEmpty ? prefix : foundPrefix) + part }
...
func capitaliseWord(_ string: String) -> String {
  ...
  let firstLetterIndex = string.distance(
    from: string.startIndex,
    to: Range(match.range, in: string)!.lowerBound
  )
...
func firstCharacter(_ string: String) -> String { return String(string.first ?? Character("")) }
